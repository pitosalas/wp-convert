---
title: "When Should You NOT Use Rails?"
author: Pito Salas
url: "http://codefol.io/posts/when-should-you-not-use-rails" 
link: "http://codefol.io/posts/when-should-you-not-use-rails" 
cover: "https://rdl.ink/render/http%3A%2F%2Fcodefol.io%2Fposts%2Fwhen-should-you-not-use-rails" 
date: 2021-04-06
tags:
    - rails
    - faq
    - how-to
---
<img class="cover" src="https://rdl.ink/render/http%3A%2F%2Fcodefol.io%2Fposts%2Fwhen-should-you-not-use-rails">

**Link: [When Should You NOT Use Rails?](http://codefol.io/posts/when-should-you-not-use-rails):** "Useful, but not necessarily universally agreed-upon, guides!"


