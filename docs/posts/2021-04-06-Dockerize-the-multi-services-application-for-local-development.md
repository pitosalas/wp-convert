---
title: "Dockerize the multi-services application for local development"
author: Pito Salas
url: "https://dev.to/amplifr/dockerize-the-multi-services-application-for-local-development-2oig" 
link: "https://dev.to/amplifr/dockerize-the-multi-services-application-for-local-development-2oig" 
cover: "https://res.cloudinary.com/practicaldev/image/fetch/s--T5lLQOxM--/c_imagga_scale,f_auto,fl_progressive,h_500,q_auto,w_1000/https://thepracticaldev.s3.amazonaws.com/i/yhvffgr58iy7s3okdqf4.jpg" 
date: 2021-04-06
tags:
    - services
    - soa
    - docker
    - scale
    - deploy
    - build
    - rails
    - how-to
---
<img class="cover" src="https://res.cloudinary.com/practicaldev/image/fetch/s--T5lLQOxM--/c_imagga_scale,f_auto,fl_progressive,h_500,q_auto,w_1000/https://thepracticaldev.s3.amazonaws.com/i/yhvffgr58iy7s3okdqf4.jpg">

**Link: [Dockerize the multi-services application for local development](https://dev.to/amplifr/dockerize-the-multi-services-application-for-local-development-2oig):** "Combining multiple services in a container makes local development easier."


