---
title: "How to install Tomcat and Java 6 on CentOS 5.2"
author: Pito Salas
url: "http://www.satollo.net/how-to-install-tomcat-and-java-6-on-centos-5-2" 
link: "http://www.satollo.net/how-to-install-tomcat-and-java-6-on-centos-5-2" 
cover: "https://rdl.ink/render/http%3A%2F%2Fwww.satollo.net%2Fhow-to-install-tomcat-and-java-6-on-centos-5-2" 
date: 2021-04-06
---
<img class="cover" src="https://rdl.ink/render/http%3A%2F%2Fwww.satollo.net%2Fhow-to-install-tomcat-and-java-6-on-centos-5-2">

**Link: [How to install Tomcat and Java 6 on CentOS 5.2](http://www.satollo.net/how-to-install-tomcat-and-java-6-on-centos-5-2):** "I'm not very smart on installing packages on Linux, but today I need to configure a Tomcat on a CentOS 5.2. The CentOS 5.2 has Tomcat 5 available as yum" 


