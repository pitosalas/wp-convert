---
title: "lbaitemple/ubuntu_server_rpi"
author: Pito Salas
url: "https://github.com/lbaitemple/ubuntu_server_rpi#step-8-gpio-run-as-non-root-devmem-no-access" 
link: "https://github.com/lbaitemple/ubuntu_server_rpi#step-8-gpio-run-as-non-root-devmem-no-access" 
cover: "https://opengraph.githubassets.com/679e7a3f2fc508233bed38ec1138f63c155fe14f078f6216a7ddafc24bf2a12b/lbaitemple/ubuntu_server_rpi" 
date: 2021-05-31
tags:
    - ubuntu
    - server
    - howto
    - raspi
---
<img class="cover" src=https://opengraph.githubassets.com/679e7a3f2fc508233bed38ec1138f63c155fe14f078f6216a7ddafc24bf2a12b/lbaitemple/ubuntu_server_rpi>

**Link: [lbaitemple/ubuntu_server_rpi](https://github.com/lbaitemple/ubuntu_server_rpi#step-8-gpio-run-as-non-root-devmem-no-access):** "Contribute to lbaitemple/ubuntu_server_rpi development by creating an account on GitHub."


