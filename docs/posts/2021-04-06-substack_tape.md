---
title: "substack/tape"
author: Pito Salas
url: "https://github.com/substack/tape" 
link: "https://github.com/substack/tape" 
cover: "https://avatars.githubusercontent.com/u/12631?s=400&v=4" 
date: 2021-04-06
---
<img class="cover" src=https://avatars.githubusercontent.com/u/12631?s=400&v=4>

**Link: [substack/tape](https://github.com/substack/tape):** "tap-producing test harness for node and browsers. Contribute to substack/tape development by creating an account on GitHub."


