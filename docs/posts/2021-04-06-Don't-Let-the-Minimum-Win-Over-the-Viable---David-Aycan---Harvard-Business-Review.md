---
title: "Don't Let the Minimum Win Over the Viable - David Aycan - Harvard Business Review"
author: Pito Salas
url: "http://blogs.hbr.org/cs/2012/05/dont_let_the_minimum_win_over.html" 
link: "http://blogs.hbr.org/cs/2012/05/dont_let_the_minimum_win_over.html" 
cover: "https://hbr.orghttps//hbr.org/resources/images/hbr_opengraph_940x490.png" 
date: 2021-04-06
tags:
    - leanstartup innovation entrepreneurs startup lean MVP
---
<img class="cover" src="https://hbr.orghttps//hbr.org/resources/images/hbr_opengraph_940x490.png">

**Link: [Don't Let the Minimum Win Over the Viable - David Aycan - Harvard Business Review](http://blogs.hbr.org/cs/2012/05/dont_let_the_minimum_win_over.html):** "In the pursuit of a minimum viable product (MVP), we've seen that it's important to evaluate early the critical components that will differentiate an offer from competition and make a product truly viable." 


