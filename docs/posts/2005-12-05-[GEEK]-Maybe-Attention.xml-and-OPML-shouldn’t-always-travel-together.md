...
**Link: [[GEEK] Maybe Attention.xml and OPML shouldn’t always travel together](None):** ""
...
they **are not one and the same** and should not be inexorably bound.  In the
**world of aggregators** , the one of the original and core use of OPML was
interoperability -- import and export of a user's set of subscriptions from
one aggregator to the next. The **potential voluminousness** of attention
information would harm OPMLs role as the universal interchange format between
aggregators. Now the approach of using an app specific namespace for attention
...
aggregators generally aren't parsing nor generating.) Technorati Tags:
