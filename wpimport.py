...
class BlogGenerator:
    ...
    def generate_drop_posts(self):
        for index, (drop_title, drop) in enumerate(self.drops.items()):
            ...
            date = self.fix_drop_z_date(drop['created'])
            if (drop['tags'] != ''):
                tags = f"""\ntags:\n    - {drop['tags']}"""
            else:
                tags = ""
    ...
    def fix_drop_z_date(self, date_string):
    ...
    def save_index_page(self):
        ...
        page_text = self.generate_index_page_title(self.cp_start_date, self.index_end_date) + "\n\n" + self.index_text
        file_path = os.path.join(self.index_directory, self.sanitize_filename(filename))
